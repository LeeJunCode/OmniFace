...
当前已经打包好了一个镜像：omniface-vlatest-docker.tar.gz
以及一个配置文件：docker-compose.yml
当获取到这两个文件后，将它们放置到一个合适的文件路径下，比如 C:/.../OmniFace/ (你需要新建一个 OmniFace 文件)
然后将 omniface-vlatest-docker.tar.gz 与 docker-compose.yml 拷贝到 C:/.../OmniFace/ 目录下
接着启动 Docker Desktop，点击左侧 Images，再打开下方的终端。
进入终端后，通过 cd 命令移动到 OmniFace 文件目录下。（注意一下命令都在 OmniFace 路径下执行）
然后执行下面的命令，进行安装镜像（这个命令只需要执行一次）

    docker load -i omniface-vlatest-docker.tar.gz

当镜像安装成功后，通过下面的命令启动镜像
